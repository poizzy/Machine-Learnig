...
class MLforKidsImageProject:
    ...
    def load_exisiting_model(self):
        try:
            training_images = self.__get_training_images_generator()
            self.num_classes = training_images.num_classes
            self.ml_class_names = self.__get_class_lookup(training_images)
            self.ml_model = tf.keras.models.load_model('model_data')
            print('use already trained, local model from dir: model_data')
        except:
            print("Local model data missing. You need to run .train_model() locally at least once before.")
